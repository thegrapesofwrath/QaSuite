# -*- coding: utf-8 -*-
"""Python Diff for Unsolved and Solved.

This module parses all python script files in an Activity folder and compares the
...
Unsolved should only contain comments or starter code from the Solved.
Comments are stripped from the code and not compared.

Example:

    $ python pydiff path/to/Activities
...
def cli(activities_directory, diff):
    ...
    for activity in activities:
        ...
        unsolved_scripts = unsolved_path.glob("**/*.py")
        solved_scripts = solved_path.glob("**/*.py")

        # Grab all of the unsolved code
        unsolved_code = set()
        for filepath in unsolved_scripts:
            with open(filepath, "r") as fh:
                lines = fh.readlines()
                for line in lines:
                    cleaned = remove_comments_and_docstrings(line)
                    if cleaned.strip():
                        unsolved_code.add(cleaned)
        solved_code = set()
        for filepath in solved_scripts:
            with open(filepath, "r") as fh:
                lines = fh.readlines()
                for line in lines:
                    cleaned = remove_comments_and_docstrings(line)
